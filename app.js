...
(function () {
  ...
  function renderSettings() {
    const lockEnabled = localStorage.getItem("mmli_lock_enabled") === "1";
    viewRoot.innerHTML = `
      <h1 class="page-title">Settings</h1>
      <p class="page-sub">Data management and device preferences.</p>
      ...
      <div class="action-list">
        <div class="action-card" style="--i:0">
          <div class="action-icon">&#128274;</div>
          <div class="action-text">
            <div class="action-title">Local Access Screen</div>
            <div class="action-sub">Convenience only — not real security</div>
          </div>
          <label style="display:flex; align-items:center;">
            <input type="checkbox" id="lock-toggle" ${lockEnabled ? "checked" : ""} style="width:20px;height:20px;" />
          </label>
        </div>
      </div>
    ...
    document.getElementById("lock-toggle").addEventListener("change", (e) => {
      if (e.target.checked) {
        openModal(`
          <h3>Set a local access code</h3>
          <p>This only hides the app behind a simple prompt on this device. It is <strong>not</strong> real security and can be bypassed by anyone with browser access. If you forget it, use "Forgot the code?" on the lock screen to reset — your school data won't be affected.</p>
          <div class="form-group"><input type="password" id="new-code" placeholder="Choose an access code" /></div>
          <div class="modal-actions">
            <button class="btn btn-outline" id="lock-cancel">Cancel</button>
            <button class="btn btn-primary" id="lock-save">Save</button>
          </div>
        `);
        ...
        document.getElementById("lock-save").addEventListener("click", () => {
          const code = document.getElementById("new-code").value.trim();
          if (!code) {
            toast("Enter a code, or tap Cancel", "error");
            return;
          }
          localStorage.setItem("mmli_lock_enabled", "1");
          localStorage.setItem("mmli_lock_code", code);
          closeModal();
          toast("Local access screen enabled", "success");
        });
      } else {
        localStorage.removeItem("mmli_lock_enabled");
        localStorage.removeItem("mmli_lock_code");
        toast("Local access screen disabled", "success");
      }
    });
  }
  ...
  /* ----------------------- Local access screen (convenience only) ----------------------- */

  function initLockScreen() {
    const enabled = localStorage.getItem("mmli_lock_enabled") === "1";
    const code = localStorage.getItem("mmli_lock_code");
    const unlockedThisSession = sessionStorage.getItem("mmli_unlocked") === "1";

    if (!enabled || unlockedThisSession) {
      appEl.hidden = false;
      return;
    }
    ...
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      if (input.value === code) {
        sessionStorage.setItem("mmli_unlocked", "1");
    ...
    document.getElementById("lock-forgot").addEventListener("click", () => {
      openModal(`
        <h3>Reset the access screen?</h3>
        <p>This removes the local access code from this device only. Your school
        records are stored separately and will <strong>not</strong> be affected.</p>
        <div class="modal-actions">
          <button class="btn btn-outline" id="forgot-cancel">Cancel</button>
          <button class="btn btn-primary" id="forgot-confirm">Reset Access Screen</button>
        </div>
      `);
      document.getElementById("forgot-cancel").addEventListener("click", closeModal);
      document.getElementById("forgot-confirm").addEventListener("click", () => {
        localStorage.removeItem("mmli_lock_enabled");
        localStorage.removeItem("mmli_lock_code");
        ...
        lockScreen.hidden = true;
        appEl.hidden = false;
        toast("Access screen reset — your data is untouched", "success");
      });
    });
  }
